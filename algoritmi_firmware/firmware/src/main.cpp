...
#if AF_SELECTOR
static BluetoothTask bluetooth_task(0, motor_task);
#endif
...
void setup() {
  ...
  motor_task.begin();
    ...
    // Connect Bluetooth task to Motor task's knob state updates
    motor_task.addListener(bluetooth_task.getKnobStateQueue());
