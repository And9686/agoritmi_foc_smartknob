...
void InterfaceTask::updateHardware() {
    ...
    /* --- AlgoritmiFOC Project --- */
        ...
        if (!leftButton && !alreadyReadLeft) {
            snprintf(buf_, sizeof(buf_), "Left Button was pressed..");
            log(buf_);
        ...
        if (!rightButton && !alreadyReadRight) {
            snprintf(buf_, sizeof(buf_), "Right Button was pressed..");
            log(buf_);
...
void InterfaceTask::processBTCommand(const String& cmd) {
    ...
    /*  
        ...
        the configurations need to be dynamically adjusted
        this means that, for example, when I switch to a volume
        setting in the APP, most likely my volume level will be at
        some random number thats not zero.

        The gig here ii to receive the command from the bluetooth
        with how many positions there are, and then adapt them here.


        *DONE* _:_ BUT firstly I need to test this positioning adaptation first!
        Now I need to change this through bluetooth, but for now I need to establish a
        data stream setting! - pending

        Possible data stream from IMX: "MODE(INT) MIN_LIMIT(INT) MAX_LIMIT(INT) ACTUAL_POSITION(INT)"
        There are some misterious errors happening when executing certain modes and values! 
        Need to keep an eye open!!!

        *DONE* _:_ ALSO can't forget that I changed the way bluetooth works and didn't test it yet!!!
        
        Also I cant forget of that incrementative strength adaptation!!! - pending

        
    */

   /* This will be the testing for the dynamical configuration adaptation */

    static PB_SmartKnobConfig changeableConfiguration = configs[0];
    ...
    /*if (cmd == "0") {
        // Here I'm trying to adapt the position from 10 to 5 
        changeableConfiguration.min_position = (int32_t)0;
        changeableConfiguration.max_position = (int32_t)20;
        applyConfig(changeableConfiguration, true);

        //current_config_ = 0;
        //applyConfig(configs[current_config_], true);
    }
    else if (cmd == "1") {
        current_config_ = (current_config_ + 1) % COUNT_OF(configs);
        applyConfig(configs[current_config_], true);
    ...
    // Optional: Log the received command
